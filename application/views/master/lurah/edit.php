<div class="modal fade" id="modal_edit">
	<div class="modal-dialog">
		<div class="modal-content">
			...
			<?= form_open('master/Kelurahan/update', ['id' => 'edit', 'class' => 'form_edit'], ['kode' => $data['kode_lurah']]) ?>
			<div class="modal-body">
				<div class="form-group">
					<label>Kode Kelurahan</label>
					<input type="text" name="kode" class="form-control" value="<?= $data['kode_lurah'] ?>" readonly="disabled">
					<span class="error kode text-red"></span>
				</div>

				<div class="form-group">
					<label>Nama Kelurahan</label>
					<input type="text" name="nama" class="form-control" value="<?= $data['nama_lurah'] ?>">
		...
		$.ajax({
			...
			success: function(response) {
				...
				if (response.status == false) {
					...
					window.location.href = "<?= site_url('lurah') ?>";
				}
			},
			complete: function() {
