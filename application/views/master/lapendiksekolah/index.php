...
<script type="text/javascript">
	$(document).ready( function(e) {
				$('.kodesekolah').select2();
let kode= "&a=" +$('.kodesekolah').val();
	          $.ajax({
                    url: '<?= site_url('master/laporan_Pendidikan_Guru_Berdasarkan_Sekolah/tabel_kode')  ?>',
                    type: "post",
                    data: kode,
                    cache: false,
...
let kode= "&a=" +$('.kodesekolah').val();
 		...
 		let kode= "/" +$('.kodesekolah').val();
                    	    setTimeout(function() {
                                window.location.href = '<?= site_url('master/laporan_Pendidikan_Guru_Berdasarkan_Sekolah/cetak')?>'+kode;
                            }, 100);
