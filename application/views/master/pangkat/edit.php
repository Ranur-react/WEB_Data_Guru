<div class="modal fade" id="modal_edit">
	<div class="modal-dialog">
		<div class="modal-content">
			...
			<?= form_open('master/Pangkat/update', ['id' => 'edit', 'class' => 'form_edit'], ['kode' => $data['kode_pangkat']]) ?>
			<div class="modal-body">
				<div class="form-group">
					<label>Kode Pangkat</label>
					<input type="text" name="kodepangkat" class="form-control" value="<?= $data['kode_pangkat'] ?>" readonly="disabled">
				...
				<div class="form-group">
					...
					<select class="form-control" name="kodegolongan">
						...
						<?php foreach ($dgolongan as $d) : ?>
							<option value="<?php echo $d['kode_golongan']; ?>" <?= $d['kode_golongan'] == $data['pangkat_kode_golongan'] ? 'selected' : null ?>><?php echo $d['kode_golongan']."-". $d['nama_golongan']; ?></option>
						<?php endforeach; ?>
					</select>
		...
		$.ajax({
			...
			success: function(response) {
				...
				if (response.status == false) {
					...
					window.location.href = "<?= site_url('pangkat') ?>";
				}
			},
			complete: function() {
